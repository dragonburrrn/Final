...
class CITY:
    def __init__(self, message, bot):
        ...
        self.gor = []
        self.abc = ['а', 'б', 'в', 'г', 'д', 'е', 'ё', 'ж', 'з', 'и', 'й', 'к', 'л', 'м', 'н', 'о', 'п', 'р', 'с', 'т', 'у', 'ф', 'х', 'ц', 'ч', 'ш', 'щ', 'ь', 'ы', 'ъ', 'э', 'ю', 'я']
        for txt in text:
            self.gor.append(txt['city'])
        ...
        if self.message.text == "Выход":
            ...
            try:
                ...
                if city in self.city_double[number1] and number_comp == number1:
                    ...
                    ans = "Игрок проиграл"
                    self.bot.send_message(self.message.chat.id, ans)
                    CITY.stop(self)
                    return

                # выбор компьютера
                if not len(self.city_double[number]) == 0:
                    random_choice = random.choice(self.city_double[number])
                    msg = self.bot.send_message(self.message.chat.id, random_choice)
                else:
                    random_choice = 'Error'
